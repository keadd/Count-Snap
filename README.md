...
- 上传图片、使用手机摄像头拍摄图片，或通过 `Ctrl+V` 粘贴剪贴板图片
- 自动描边并检测大量重复出现、轮廓和大小相近的色块
- 智能比较自动色块和重复轮廓结果，默认采用质量更高的一组
- 标出两种策略的一致区域和独有区域，可直接采用备选策略中的识别框
- 默认要求至少出现 8 个相似区域，也可以手动调整
- 支持在多个候选重复组之间切换
- 可点击一个目标零件，重新寻找与它相似的区域
- 可框选检测范围，排除范围外的背景和杂物
- 支持撤销、重做和恢复自动检测结果
- 支持基础轮廓检测模式
- 自动显示总数和识别区域数量
...
默认模式。后端会同时运行自动色块和重复轮廓检测，并根据内嵌框、重叠框、区域数量和面积一致性计算质量分，自动采用更可靠的结果。

点击目标或框选检测范围时，两种策略仍会同时运行。系统会优先采用真正匹配目标且质量更高的结果，并在结果复核区显示两种策略的一致区域、主结果独有区域和备选结果独有区域。

### 重复轮廓检测

默认模式。后端会先提取候选色块并描边，再综合比较轮廓、面积、颜色和长宽比，把大量重复出现的相似区域归为同一组。默认只有数量达到 8 个的重复组才会自动计数。

检测完成后可以切换不同候选组，也可以点击图片中的一个零件作为参考目标。对于背景复杂的照片，可以先框选检测范围再运行检测。
...
| `mode` | string | `smart` | 检测模式，支持 `smart`、`repeat_contours`、`auto_color_blocks`、`color`、`basic` |
| `min_area` | number | `900` | 最小区域面积 |
| `min_repeat` | number | `8` | 重复轮廓模式要求的最低相似区域数量 |
| `target_x` | number | 可选 | 参考目标在原图中的横坐标 |
| `target_y` | number | 可选 | 参考目标在原图中的纵坐标 |
| `roi_x` | number | 可选 | 检测范围左上角横坐标 |
| `roi_y` | number | 可选 | 检测范围左上角纵坐标 |
| `roi_width` | number | 可选 | 检测范围宽度 |
| `roi_height` | number | 可选 | 检测范围高度 |
| `threshold` | number | `0` | 阈值，`0` 表示自动阈值 |
| `blur` | number | `7` | 模糊半径 |
| `invert` | boolean | `true` | 是否反转前景 |

响应中会返回图片尺寸、检测区域、总数、候选颜色组、候选重复组及其完整识别框等信息。智能模式还会返回 `selectedStrategy`、`strategyScores` 和两种策略的候选数量。
